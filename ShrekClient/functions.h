...
jfieldID entityHitf;
jfieldID currentScreenf;
jfieldID allowUserInputf;
...
jobject getMouseOver(float p_78473_1_, JNIEnv* env, double reach)
{
	if (renderViewEntity != NULL)
	{
		if (theWorld != NULL)
		{
			...
			if (env->GetObjectField(EntityRenderer,pointedEntityf) != NULL && var12 > 3.0f)
			{
				...
				return env->GetObjectField(env->NewObject(MovingObjectPosition, movingObjectPositionc, env->GetObjectField(EntityRenderer, pointedEntityf), var9),entityHitf);
				cout << env->GetObjectField(mc, objectMouseOverf) << endl;
...
void postPreInit(JNIEnv* env)
{
	...
	GuiScreen = getObject(env, "net.minecraft.client.gui.GuiScreen");
	...
	currentScreenf = env->GetFieldID(mcClass, "field_71462_r", "Lnet/minecraft/client/gui/GuiScreen;");
	allowUserInputf = env->GetFieldID(GuiScreen, "field_146291_p", "Z");
	
	//objects
	renderViewEntity = env->GetObjectField(mc, renderViewEntityf);
	currentScreen = env->GetObjectField(mc, currentScreenf);
	allowUserInput = env->GetBooleanField(currentScreen, allowUserInputf);
	...
	swingItemf = env->GetMethodID(EntityPlayer, "func_71038_i", "()V");
